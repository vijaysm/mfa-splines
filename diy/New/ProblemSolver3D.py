...
class ProblemSolver3D:
    ...
    def __init__(self, icBlock, coreb, xb, xl, yl, zl, degree, augmentSpanSpace=0, useDiagonalBlocks=True):
        icBlock.xbounds = [xb.min[0], xb.max[0], xb.min[1], xb.max[1], xb.min[2], xb.max[2]]
    ...
    def compute_decode_operators(self, RN):
        for dir in ['x', 'y', 'z']:
            RN[dir] = self.inputCB.NUVW[dir] / np.sum(self.inputCB.NUVW[dir], axis=1)[:, np.newaxis]

    def decode(self, P, RN):
        # DD = np.matmul(np.matmul(np.matmul(RN['x'], P), RN['y'].T).T, RN['z'].T)
        DD = np.einsum('ijk,lk->ijl', np.einsum('ijk,lj->ilk', np.einsum('ijk,li->ljk', P, RN['x']), RN['y']), RN['z'])
    ...
    def lsqFit(self):
        ...
        NTNxInv = np.linalg.inv(np.matmul(self.inputCB.decodeOpXYZ['x'].T, self.inputCB.decodeOpXYZ['x']))
        NTNyInv = np.linalg.inv(np.matmul(self.inputCB.decodeOpXYZ['y'].T, self.inputCB.decodeOpXYZ['y']))
        NTNzInv = np.linalg.inv(np.matmul(self.inputCB.decodeOpXYZ['z'].T, self.inputCB.decodeOpXYZ['z']))
        ...
        NxTQNy = np.einsum('ijk,il->ljk', np.einsum('ijk,jl->ilk', np.einsum('ijk,kl->ijl', self.inputCB.refSolutionLocal, self.inputCB.decodeOpXYZ['z']), self.inputCB.decodeOpXYZ['y']), self.inputCB.decodeOpXYZ['x'])
    ...
    def update_bounds(self):
        return [
            ...
            self.inputCB.corebounds[1][1], self.inputCB.corebounds[2][0], self.inputCB.corebounds[2][1]]
    ...
    def send_diy(self, inputCB, cp):
        ...
        for i in range(len(link)):
            ...
            if len(inputCB.controlPointData):
                ...
                direction = (dir[2]+1)*self.dimension**2 + (dir[1]+1)*self.dimension + (dir[0]+1)
                print("sending: ", cp.gid(), "to", target.gid, '-- Dir = ', dir, 'Direction = ', direction)
                ...
                if dir[2] == 0: # same z-height layer
                    ...
                    if dir[0] == 0:  # target is coupled in Y-direction
                        if dir[1] > 0:  # target block is above current subdomain
                            ...
                            cp.enqueue(
                                target, inputCB.knotsAdaptive['y'][-1:-2-self.degree-self.augmentSpanSpace:-1])

                        else:  # target block is below current subdomain
                            cp.enqueue(target, inputCB.controlPointData[:, :loffset, :])
                            # cp.enqueue(target, inputCB.controlPointData)
                            cp.enqueue(
                                target, inputCB.knotsAdaptive['y'][0:1+self.degree+self.augmentSpanSpace])
                    ...
                    # target is coupled in X-direction
                        ...
                        if self.useDiagonalBlocks:
                            # target block is diagonally top right to current subdomain
                            if dir[0] > 0 and dir[1] > 0:
                                cp.enqueue(target, inputCB.controlPointData[-loffset:,-loffset:, -loffset:])
                    ...
                    if dir[0] == 0:  # target is coupled in Y-direction
                        if dir[1] == 0:  # target block is directly below in z-direction (but same x-y)
                            cp.enqueue(target, inputCB.controlPointData[:, :, :loffset])
                            # cp.enqueue(target, inputCB.controlPointData)
                            cp.enqueue(target, inputCB.knotsAdaptive['z'][0:1+self.degree+self.augmentSpanSpace])
                            # cp.enqueue(
                            #     target, inputCB.knotsAdaptive['y'][-1:-2-degree-augmentSpanSpace:-1])
    ...
    def initialize_solution(self, inputCB, initSol, degree, augmentSpanSpace, fullyPinned):
        ...
        if fullyPinned:
            ...
            freeBounds[0] = 0 if inputCB.isClamped['left'] else (
                nconstraints-1 if oddDegree else nconstraints)
            freeBounds[1] = initSol.shape[0] if inputCB.isClamped['right'] else initSol.shape[0] - (nconstraints - 1 if oddDegree else nconstraints)

            freeBounds[2] = 0 if inputCB.isClamped['bottom'] else(
                nconstraints - 1 if oddDegree else nconstraints)
            freeBounds[3] = initSol.shape[1] if inputCB.isClamped['top'] else initSol.shape[1]-(nconstraints-1 if oddDegree else nconstraints)

            freeBounds[4] = 0 if inputCB.isClamped['down'] else(
                nconstraints - 1 if oddDegree else nconstraints)
            freeBounds[5] = initSol.shape[2] if inputCB.isClamped['up'] else initSol.shape[2]-(nconstraints-1 if oddDegree else nconstraints)

            if 'left' in inputCB.boundaryConstraints:
                if oddDegree:
                    localBCAssembly[nconstraints-1, freeBounds[2]:freeBounds[3],
                                    freeBounds[4]:freeBounds[5]] += inputCB.boundaryConstraints['left'][-nconstraints, freeBounds[2]:freeBounds[3], freeBounds[4]:freeBounds[5]]
                    ...
                    if nconstraints > 1:
                        initSol[:nconstraints-1, freeBounds[2]:freeBounds[3], freeBounds[4]:freeBounds[5]
                                ] = inputCB.boundaryConstraints['left'][-degree - loffset:-nconstraints, freeBounds[2]:freeBounds[3], freeBounds[4]:freeBounds[5]]
                        localAssemblyWeights[:nconstraints-1,
                                            freeBounds[2]:freeBounds[3],freeBounds[4]:freeBounds[5]] += 1.0
                else:
            ...
            if 'right' in inputCB.boundaryConstraints:
                if oddDegree:
                    localBCAssembly[-nconstraints, freeBounds[2]:freeBounds[3], freeBounds[4]:freeBounds[5]
                                    ] += inputCB.boundaryConstraints['right'][nconstraints-1, freeBounds[2]:freeBounds[3], freeBounds[4]:freeBounds[5]]
                    localAssemblyWeights[-nconstraints,
                                        freeBounds[2]:freeBounds[3], freeBounds[4]:freeBounds[5]] += 1.0

                    if nconstraints > 1:
                        initSol[-nconstraints + 1:, freeBounds[2]:freeBounds[3], freeBounds[4]:freeBounds[5]
                                ] = inputCB.boundaryConstraints['right'][nconstraints: degree + loffset, freeBounds[2]:freeBounds[3], freeBounds[4]:freeBounds[5]]
                        localAssemblyWeights[-nconstraints + 1:,
                                            freeBounds[2]:freeBounds[3], freeBounds[4]:freeBounds[5]] += 1.0
                else:
                    localAssemblyWeights[-nconstraints:,
                                        freeBounds[2]:freeBounds[3], freeBounds[4]:freeBounds[5]] += 1.0
                    initSol[-nconstraints:, freeBounds[2]: freeBounds[3], freeBounds[4]:freeBounds[5]] = inputCB.boundaryConstraints['right'][
                        nconstraints: degree + loffset, freeBounds[2]: freeBounds[3], freeBounds[4]:freeBounds[5]]

            if 'top' in inputCB.boundaryConstraints:
                if oddDegree:
                    localBCAssembly[freeBounds[0]: freeBounds[1],
                                    -nconstraints, freeBounds[4]:freeBounds[5]] += inputCB.boundaryConstraints['top'][
                    ...
                    if nconstraints > 1:
                        initSol[freeBounds[0]: freeBounds[1],
                                -nconstraints + 1:, freeBounds[4]:freeBounds[5]] = inputCB.boundaryConstraints['top'][
                            freeBounds[0]: freeBounds[1],
                            nconstraints: loffset + degree, freeBounds[4]:freeBounds[5]]
                        localAssemblyWeights[freeBounds[0]
                            :freeBounds[1], -nconstraints+1:, freeBounds[4]:freeBounds[5]] += 1.0
                else:
                    initSol[freeBounds[0]:freeBounds[1], -nconstraints:, freeBounds[4]:freeBounds[5]] = inputCB.boundaryConstraints['top'][
                        freeBounds[0]:freeBounds[1], nconstraints:loffset+degree, freeBounds[4]:freeBounds[5]]
                    localAssemblyWeights[freeBounds[0]
                        :freeBounds[1], -nconstraints:, freeBounds[4]:freeBounds[5]] += 1.0
            ...
            if 'bottom' in inputCB.boundaryConstraints:
                if oddDegree:
                    localBCAssembly[freeBounds[0]:freeBounds[1],
                                    nconstraints-1, freeBounds[4]:freeBounds[5]] += inputCB.boundaryConstraints['bottom'][freeBounds[0]:freeBounds[1], -nconstraints, freeBounds[4]:freeBounds[5]]
                    localAssemblyWeights[freeBounds[0]
                        :freeBounds[1], nconstraints-1, freeBounds[4]:freeBounds[5]] += 1.0

                    if nconstraints > 1:
                        initSol[freeBounds[0]: freeBounds[1],
                                : nconstraints - 1, freeBounds[4]:freeBounds[5]] = inputCB.boundaryConstraints['bottom'][
                            freeBounds[0]: freeBounds[1],
                    ...
                    initSol[freeBounds[0]: freeBounds[1],
                            : nconstraints, freeBounds[4]:freeBounds[5]] = inputCB.boundaryConstraints['bottom'][
                        freeBounds[0]: freeBounds[1],
                        -degree - loffset: -nconstraints, freeBounds[4]:freeBounds[5]]
                    localAssemblyWeights[freeBounds[0]
                        :freeBounds[1], : nconstraints, freeBounds[4]:freeBounds[5]] += 1.0
            ...
            if 'down' in inputCB.boundaryConstraints:
                if oddDegree:
                    localBCAssembly[freeBounds[0]: freeBounds[1], freeBounds[2]: freeBounds[3],
                                    nconstraints - 1] += inputCB.boundaryConstraints['down'][
                    ...
                    initSol[freeBounds[0]:freeBounds[1], freeBounds[2]: freeBounds[3], : nconstraints] = inputCB.boundaryConstraints['down'][freeBounds[0]                                                                                                                   :freeBounds[1], freeBounds[2]: freeBounds[3], -degree - loffset: -nconstraints]
                    localAssemblyWeights[freeBounds[0]
            ...
            if 'up' in inputCB.boundaryConstraints:
                if oddDegree:
                    localBCAssembly[freeBounds[0]:freeBounds[1], freeBounds[2]: freeBounds[3],
                                    -nconstraints] += inputCB.boundaryConstraints['up'][freeBounds[0]:freeBounds[1], freeBounds[2]: freeBounds[3], nconstraints-1]
                    localAssemblyWeights[freeBounds[0]
                        :freeBounds[1], freeBounds[2]: freeBounds[3], -nconstraints] += 1.0
                    ...
                    initSol[freeBounds[0]: freeBounds[1], freeBounds[2]: freeBounds[3],
                        -nconstraints:] = inputCB.boundaryConstraints['up'][
                        freeBounds[0]: freeBounds[1], freeBounds[2]: freeBounds[3],
                        nconstraints:loffset+degree]
                    localAssemblyWeights[freeBounds[0]
                        :freeBounds[1], freeBounds[2]: freeBounds[3], -nconstraints:] += 1.0
            ...
            if 'top-left' in inputCB.boundaryConstraints:
                if oddDegree:
                    localBCAssembly[nconstraints-1, -
                                    nconstraints, :] += inputCB.boundaryConstraints['top-left'][-nconstraints, nconstraints-1, :]
                    localAssemblyWeights[nconstraints -
                                        1, -nconstraints, :] += 1.0
                    ...
                    if nconstraints > 1:
                        ...
                        initSol[: nconstraints - 1, -nconstraints + 1:, :] = inputCB.boundaryConstraints['top-left'][-degree -
                                                                                                                        loffset: -nconstraints, nconstraints: degree + loffset, :]
                else:
                    initSol[: nconstraints, -nconstraints:, :] = inputCB.boundaryConstraints['top-left'][-degree -
                                                                                                loffset: -nconstraints, nconstraints: degree + loffset, :]
                    localAssemblyWeights[: nconstraints, -
                                        nconstraints:, :] += 1.0
            ...
            if 'bottom-right' in inputCB.boundaryConstraints:
                if oddDegree:
                    localBCAssembly[-nconstraints,
                                    nconstraints-1, :] += inputCB.boundaryConstraints['bottom-right'][nconstraints-1, -nconstraints, :]
                    localAssemblyWeights[-nconstraints,
                                        nconstraints-1, :] += 1.0

                    if nconstraints > 1:
                        # assert(freeBounds[2] == nconstraints - 1)
                        initSol[-nconstraints + 1:, : nconstraints - 1, :] = inputCB.boundaryConstraints['bottom-right'][
                            nconstraints: degree + loffset, -degree - loffset: -nconstraints, :]
                        localAssemblyWeights[-nconstraints +
                                            1:, : nconstraints - 1, :] += 1.0
                else:
                    initSol[-nconstraints:, : nconstraints, :] = inputCB.boundaryConstraints['bottom-right'][
                        nconstraints: degree + loffset, -degree - loffset: -nconstraints, :]
                    localAssemblyWeights[-nconstraints:,
                                        : nconstraints, :] += 1.0
            ...
            if 'bottom-left' in inputCB.boundaryConstraints:
                if oddDegree:
                    localBCAssembly[nconstraints-1,
                                    nconstraints-1, :] += inputCB.boundaryConstraints['bottom-left'][-nconstraints, -nconstraints, :]
                    localAssemblyWeights[nconstraints -
                                        1, nconstraints-1, :] += 1.0

                    if nconstraints > 1:
                        # assert(freeBounds[0] == nconstraints - 1)
                        initSol[: nconstraints - 1, : nconstraints - 1, :] = inputCB.boundaryConstraints['bottom-left'][-degree -
                                                                                                                loffset: -nconstraints, -degree - loffset: -nconstraints, :]
                        localAssemblyWeights[: nconstraints -
                                            1, : nconstraints - 1, :] += 1.0
                else:
                    initSol[: nconstraints, : nconstraints, :] = inputCB.boundaryConstraints['bottom-left'][-degree -
                                                                                                    loffset: -nconstraints, -degree - loffset: -nconstraints, :]
                    localAssemblyWeights[: nconstraints,
                                        : nconstraints, :] += 1.0
            ...
            if 'top-right' in inputCB.boundaryConstraints:
                if oddDegree:
                    localBCAssembly[-nconstraints, -nconstraints, :] += inputCB.boundaryConstraints['top-right'][
                        nconstraints - 1, nconstraints - 1, :]
                    localAssemblyWeights[-nconstraints, -
                                        nconstraints, :] += 1.0

                    if nconstraints > 1:
                        initSol[-nconstraints + 1:, -nconstraints + 1:, :] = inputCB.boundaryConstraints['top-right'][
                            nconstraints: degree + loffset, nconstraints: degree + loffset, :]
                        localAssemblyWeights[-nconstraints +
                                            1:, -nconstraints + 1:, :] += 1.0
                else:
                    initSol[-nconstraints:, -nconstraints:, :] = inputCB.boundaryConstraints['top-right'][
                        nconstraints: degree + loffset, nconstraints: degree + loffset, :]
                    localAssemblyWeights[-nconstraints:, -
                                        nconstraints:, :] += 1.0
            ...
            if 'up-top' in inputCB.boundaryConstraints:
                if oddDegree:
                    localBCAssembly[freeBounds[0]: freeBounds[1],
                                    -nconstraints, -nconstraints] += inputCB.boundaryConstraints['up-top'][
                        freeBounds[0]: freeBounds[1],
                        nconstraints - 1, nconstraints-1]
                    localAssemblyWeights[freeBounds[0]
                        :freeBounds[1], -nconstraints, -nconstraints] += 1.0

                    if nconstraints > 1:
                        initSol[freeBounds[0]: freeBounds[1],
                                -nconstraints + 1:, -nconstraints + 1:] = inputCB.boundaryConstraints['up-top'][
                            freeBounds[0]: freeBounds[1],
                            nconstraints: loffset + degree, nconstraints: loffset + degree]
                        localAssemblyWeights[freeBounds[0]
                            :freeBounds[1], -nconstraints+1:, -nconstraints + 1:] += 1.0
                else:
                    initSol[freeBounds[0]:freeBounds[1], -nconstraints:, -nconstraints:] = inputCB.boundaryConstraints['up-top'][
                                    freeBounds[0]:freeBounds[1], nconstraints:loffset+degree, nconstraints:loffset+degree]
                    localAssemblyWeights[freeBounds[0]
                        :freeBounds[1], -nconstraints:, -nconstraints:] += 1.0
            ...
            if 'up-bottom' in inputCB.boundaryConstraints:
                if oddDegree:
                    localBCAssembly[freeBounds[0]: freeBounds[1],
                                    nconstraints-1, -nconstraints] += inputCB.boundaryConstraints['up-bottom'][
                        freeBounds[0]: freeBounds[1],
                        nconstraints - 1, nconstraints-1]
                    localAssemblyWeights[freeBounds[0]
                        :freeBounds[1], nconstraints-1, -nconstraints] += 1.0

                    if nconstraints > 1:
                        initSol[freeBounds[0]: freeBounds[1],
                                : nconstraints - 1, -nconstraints + 1:] = inputCB.boundaryConstraints['up-bottom'][
                            freeBounds[0]: freeBounds[1],
                            nconstraints: loffset + degree, nconstraints: loffset + degree]
                        localAssemblyWeights[freeBounds[0]
                            :freeBounds[1], : nconstraints - 1, -nconstraints + 1:] += 1.0
                else:
                    initSol[freeBounds[0]:freeBounds[1], : nconstraints, -nconstraints:] = inputCB.boundaryConstraints['up-bottom'][
                                    freeBounds[0]:freeBounds[1], nconstraints:loffset+degree, nconstraints:loffset+degree]
                    localAssemblyWeights[freeBounds[0]
                        :freeBounds[1], : nconstraints, -nconstraints:] += 1.0
            ...
            if 'down-top' in inputCB.boundaryConstraints:
                if oddDegree:
                    localBCAssembly[freeBounds[0]: freeBounds[1],
                                    -nconstraints, nconstraints-1] += inputCB.boundaryConstraints['down-top'][
                        freeBounds[0]: freeBounds[1],
                        nconstraints - 1, -nconstraints]
                    localAssemblyWeights[freeBounds[0]
                        :freeBounds[1], -nconstraints, nconstraints-1] += 1.0

                    if nconstraints > 1:
                        initSol[freeBounds[0]: freeBounds[1],
                                -nconstraints + 1:, : nconstraints - 1] = inputCB.boundaryConstraints['down-top'][
                            freeBounds[0]: freeBounds[1],
                            nconstraints: loffset + degree, -degree - loffset: -nconstraints]
                        localAssemblyWeights[freeBounds[0]
                            :freeBounds[1], -nconstraints+1:, : nconstraints - 1] += 1.0
                else:
                    initSol[freeBounds[0]:freeBounds[1], -nconstraints:, :nconstraints] = inputCB.boundaryConstraints['down-top'][
                                    freeBounds[0]:freeBounds[1], nconstraints:loffset+degree, -degree - loffset: -nconstraints]
                    localAssemblyWeights[freeBounds[0]
                        :freeBounds[1], -nconstraints:, :nconstraints] += 1.0
            ...
            if 'down-bottom' in inputCB.boundaryConstraints:
                if oddDegree:
                    localBCAssembly[freeBounds[0]: freeBounds[1],
                                    nconstraints-1, nconstraints-1] += inputCB.boundaryConstraints['down-bottom'][
                        freeBounds[0]: freeBounds[1],
                        nconstraints - 1, -nconstraints]
                    localAssemblyWeights[freeBounds[0]
                        :freeBounds[1], nconstraints-1, nconstraints-1] += 1.0
                    ...
                    initSol[freeBounds[0]:freeBounds[1], : nconstraints, :nconstraints] = inputCB.boundaryConstraints['down-bottom'][
                                    freeBounds[0]:freeBounds[1], nconstraints:loffset+degree, -degree - loffset: -nconstraints]
                    localAssemblyWeights[freeBounds[0]
                        :freeBounds[1], : nconstraints, :nconstraints] += 1.0
            ...
            if 'up-right' in inputCB.boundaryConstraints:
                if oddDegree:
                    localBCAssembly[-nconstraints, freeBounds[2]: freeBounds[3], -nconstraints] += inputCB.boundaryConstraints['up-right'][
                        nconstraints - 1, freeBounds[2]: freeBounds[3], nconstraints-1]
                    localAssemblyWeights[-nconstraints, freeBounds[2]: freeBounds[3], -nconstraints] += 1.0

                    if nconstraints > 1:
                        initSol[-nconstraints + 1:, freeBounds[2]: freeBounds[3], -nconstraints + 1:] = inputCB.boundaryConstraints['up-right'][
                            nconstraints: loffset + degree, freeBounds[2]: freeBounds[3], nconstraints: loffset + degree]
                        localAssemblyWeights[-nconstraints+1:, freeBounds[2]: freeBounds[3], -nconstraints + 1:] += 1.0
                else:
                    initSol[ -nconstraints:, freeBounds[2]: freeBounds[3], -nconstraints:] = inputCB.boundaryConstraints['up-right'][
                                    nconstraints:loffset+degree, freeBounds[2]: freeBounds[3], nconstraints:loffset+degree]
                    localAssemblyWeights[-nconstraints:, freeBounds[2]: freeBounds[3], -nconstraints:] += 1.0
            ...
            if 'up-left' in inputCB.boundaryConstraints:
                if oddDegree:
                    localBCAssembly[nconstraints-1, freeBounds[2]: freeBounds[3], -nconstraints] += inputCB.boundaryConstraints['up-left'][
                        nconstraints - 1, freeBounds[2]: freeBounds[3], nconstraints-1]
                    localAssemblyWeights[nconstraints-1, freeBounds[2]: freeBounds[3], -nconstraints] += 1.0

                    if nconstraints > 1:
                        initSol[: nconstraints - 1, freeBounds[2]: freeBounds[3], -nconstraints + 1:] = inputCB.boundaryConstraints['up-left'][
                            nconstraints: loffset + degree, freeBounds[2]: freeBounds[3], nconstraints: loffset + degree]
                        localAssemblyWeights[: nconstraints - 1, freeBounds[2]: freeBounds[3], -nconstraints + 1:] += 1.0
                else:
                    initSol[: nconstraints, freeBounds[2]: freeBounds[3], -nconstraints:] = inputCB.boundaryConstraints['up-left'][
                                    nconstraints:loffset+degree, freeBounds[2]: freeBounds[3], nconstraints:loffset+degree]
                    localAssemblyWeights[: nconstraints, freeBounds[2]: freeBounds[3], -nconstraints:] += 1.0
            ...
            if 'down-right' in inputCB.boundaryConstraints:
                if oddDegree:
                    localBCAssembly[-nconstraints, freeBounds[2]: freeBounds[3], nconstraints-1] += inputCB.boundaryConstraints['down-right'][
                        nconstraints - 1, freeBounds[2]: freeBounds[3], -nconstraints]
                    localAssemblyWeights[-nconstraints, freeBounds[2]: freeBounds[3], nconstraints-1] += 1.0

                    if nconstraints > 1:
                        initSol[-nconstraints + 1:, freeBounds[2]: freeBounds[3], : nconstraints - 1] = inputCB.boundaryConstraints['down-right'][
                            nconstraints: loffset + degree, freeBounds[2]: freeBounds[3], -degree - loffset: -nconstraints]
                        localAssemblyWeights[ -nconstraints+1:, freeBounds[2]: freeBounds[3], : nconstraints - 1] += 1.0
                else:
                    initSol[-nconstraints:, freeBounds[2]: freeBounds[3], :nconstraints] = inputCB.boundaryConstraints['down-right'][
                                    nconstraints:loffset+degree, freeBounds[2]: freeBounds[3], -degree - loffset: -nconstraints]
                    localAssemblyWeights[ -nconstraints:, freeBounds[2]: freeBounds[3], :nconstraints] += 1.0
            ...
            if 'down-left' in inputCB.boundaryConstraints:
                if oddDegree:
                    localBCAssembly[nconstraints-1, freeBounds[2]: freeBounds[3], nconstraints-1] += inputCB.boundaryConstraints['down-left'][
                        nconstraints - 1, freeBounds[2]: freeBounds[3], -nconstraints]
                    localAssemblyWeights[nconstraints-1, freeBounds[2]: freeBounds[3], nconstraints-1] += 1.0

                    if nconstraints > 1:
                        initSol[: nconstraints - 1, freeBounds[2]: freeBounds[3], : nconstraints - 1] = inputCB.boundaryConstraints['down-left'][
                            nconstraints: loffset + degree, freeBounds[2]: freeBounds[3], -degree - loffset: -nconstraints]
                        localAssemblyWeights[: nconstraints - 1, freeBounds[2]: freeBounds[3], : nconstraints - 1] += 1.0
                else:
                    initSol[: nconstraints, freeBounds[2]: freeBounds[3], :nconstraints] = inputCB.boundaryConstraints['down-left'][
                                    nconstraints:loffset+degree, freeBounds[2]: freeBounds[3], -degree - loffset: -nconstraints]
                    localAssemblyWeights[ : nconstraints, freeBounds[2]: freeBounds[3], :nconstraints] += 1.0
            ...
            if 'up-top-left' in inputCB.boundaryConstraints:
                if oddDegree:
                    localBCAssembly[nconstraints-1, -
                                    nconstraints, -nconstraints] += inputCB.boundaryConstraints['up-top-left'][-nconstraints, nconstraints-1, nconstraints - 1]
                    localAssemblyWeights[nconstraints -
                                        1, -nconstraints, -nconstraints] += 1.0
                    ...
                    if nconstraints > 1:
                        ...
                        localAssemblyWeights[: nconstraints -
                                            1, -nconstraints + 1:, :] += 1.0
                        initSol[: nconstraints - 1, -nconstraints + 1:, -nconstraints + 1:] = inputCB.boundaryConstraints['up-top-left'][-degree -
                                                                                                                        loffset: -nconstraints, nconstraints: degree + loffset, nconstraints: loffset + degree]
                else:
                    initSol[: nconstraints, -nconstraints:, -nconstraints:] = inputCB.boundaryConstraints['up-top-left'][-degree -
                                                                                                loffset: -nconstraints, nconstraints: degree + loffset, nconstraints:loffset+degree]
                    localAssemblyWeights[: nconstraints, -
                                        nconstraints:, -nconstraints:] += 1.0

            if 'up-bottom-right' in inputCB.boundaryConstraints:
                if oddDegree:
                    localBCAssembly[-nconstraints,
                                    nconstraints-1, -nconstraints] += inputCB.boundaryConstraints['up-bottom-right'][nconstraints-1, -nconstraints, nconstraints - 1]
                    localAssemblyWeights[-nconstraints,
                                        nconstraints-1, -nconstraints] += 1.0

                    if nconstraints > 1:
                        # assert(freeBounds[2] == nconstraints - 1)
                        initSol[-nconstraints + 1:, : nconstraints - 1, -nconstraints + 1:] = inputCB.boundaryConstraints['up-bottom-right'][
                            nconstraints: degree + loffset, -degree - loffset: -nconstraints, nconstraints: loffset + degree]
                        localAssemblyWeights[-nconstraints +
                                            1:, : nconstraints - 1, -nconstraints + 1:] += 1.0
                else:
                    initSol[-nconstraints:, : nconstraints, -nconstraints:] = inputCB.boundaryConstraints['up-bottom-right'][
                        nconstraints: degree + loffset, -degree - loffset: -nconstraints, nconstraints:loffset+degree]
                    localAssemblyWeights[-nconstraints:,
                                        : nconstraints, -nconstraints:] += 1.0
            ...
            if 'up-bottom-left' in inputCB.boundaryConstraints:
                if oddDegree:
                    localBCAssembly[nconstraints-1,
                                    nconstraints-1, -nconstraints] += inputCB.boundaryConstraints['up-bottom-left'][-nconstraints, -nconstraints, nconstraints - 1]
                    localAssemblyWeights[nconstraints -
                                        1, nconstraints-1, -nconstraints] += 1.0

                    if nconstraints > 1:
                        # assert(freeBounds[0] == nconstraints - 1)
                        initSol[: nconstraints - 1, : nconstraints - 1, -nconstraints + 1:] = inputCB.boundaryConstraints['up-bottom-left'][-degree -
                                                                                                                loffset: -nconstraints, -degree - loffset: -nconstraints, nconstraints: loffset + degree]
                        localAssemblyWeights[: nconstraints -
                                            1, : nconstraints - 1, -nconstraints + 1:] += 1.0
                else:
                    initSol[: nconstraints, : nconstraints, -nconstraints:] = inputCB.boundaryConstraints['up-bottom-left'][-degree -
                                                                                                    loffset: -nconstraints, -degree - loffset: -nconstraints, nconstraints:loffset+degree]
                    localAssemblyWeights[: nconstraints,
                                        : nconstraints, -nconstraints:] += 1.0
            ...
            if 'up-top-right' in inputCB.boundaryConstraints:
                if oddDegree:
                    localBCAssembly[-nconstraints, -nconstraints, -nconstraints] += inputCB.boundaryConstraints['up-top-right'][
                        nconstraints - 1, nconstraints - 1, nconstraints - 1]
                    localAssemblyWeights[-nconstraints, -
                                        nconstraints, -nconstraints] += 1.0

                    if nconstraints > 1:
                        initSol[-nconstraints + 1:, -nconstraints + 1:, -nconstraints + 1:] = inputCB.boundaryConstraints['up-top-right'][
                            nconstraints: degree + loffset, nconstraints: degree + loffset, nconstraints: loffset + degree]
                        localAssemblyWeights[-nconstraints +
                                            1:, -nconstraints + 1:, -nconstraints + 1:] += 1.0
                else:
                    initSol[-nconstraints:, -nconstraints:, -nconstraints:] = inputCB.boundaryConstraints['up-top-right'][
                        nconstraints: degree + loffset, nconstraints: degree + loffset, nconstraints:loffset+degree]
                    localAssemblyWeights[-nconstraints:, -
                                        nconstraints:, -nconstraints:] += 1.0
            ...
            if 'down-top-left' in inputCB.boundaryConstraints:
                if oddDegree:
                    localBCAssembly[nconstraints-1, -
                                    nconstraints, nconstraints - 1] += inputCB.boundaryConstraints['down-top-left'][-nconstraints, nconstraints-1, -nconstraints]
                    localAssemblyWeights[nconstraints -
                                        1, -nconstraints, nconstraints - 1] += 1.0
                    ...
                    if nconstraints > 1:
                        ...
                        localAssemblyWeights[: nconstraints -
                                            1, -nconstraints + 1:, : nconstraints - 1] += 1.0
                        localBCAssembly[: nconstraints - 1, -nconstraints + 1:, : nconstraints - 1] += inputCB.boundaryConstraints['down-top-left'][-degree -
                                                                                                                        loffset: -nconstraints, nconstraints: degree + loffset, -degree - loffset: -nconstraints]
                else:
                    initSol[: nconstraints, -nconstraints:, : nconstraints] = inputCB.boundaryConstraints['down-top-left'][-degree -
                                                                                                loffset: -nconstraints, nconstraints: degree + loffset, -degree - loffset: -nconstraints]
                    localAssemblyWeights[: nconstraints, -
                                        nconstraints:, : nconstraints] += 1.0
            ...
            if 'down-bottom-right' in inputCB.boundaryConstraints:
                if oddDegree:
                    localBCAssembly[-nconstraints,
                                    nconstraints-1, nconstraints - 1] += inputCB.boundaryConstraints['down-bottom-right'][nconstraints-1, -nconstraints, -nconstraints]
                    localAssemblyWeights[-nconstraints,
                                        nconstraints-1, nconstraints - 1] += 1.0

                    if nconstraints > 1:
                        # assert(freeBounds[2] == nconstraints - 1)
                        initSol[-nconstraints + 1:, : nconstraints - 1, : nconstraints - 1] = inputCB.boundaryConstraints['down-bottom-right'][
                            nconstraints: degree + loffset, -degree - loffset: -nconstraints, -degree - loffset: -nconstraints]
                        localAssemblyWeights[-nconstraints +
                                            1:, : nconstraints - 1, : nconstraints - 1] += 1.0
                else:
                    initSol[-nconstraints:, : nconstraints, : nconstraints] = inputCB.boundaryConstraints['down-bottom-right'][
                        nconstraints: degree + loffset, -degree - loffset: -nconstraints, -degree - loffset: -nconstraints]
                    localAssemblyWeights[-nconstraints:,
                                        : nconstraints, : nconstraints] += 1.0
            ...
            if 'down-bottom-left' in inputCB.boundaryConstraints:
                if oddDegree:
                    localBCAssembly[nconstraints-1,
                                    nconstraints-1, nconstraints - 1] += inputCB.boundaryConstraints['down-bottom-left'][-nconstraints, -nconstraints, -nconstraints]
                    localAssemblyWeights[nconstraints -
                                        1, nconstraints-1, nconstraints - 1] += 1.0

                    if nconstraints > 1:
                        # assert(freeBounds[0] == nconstraints - 1)
                        initSol[: nconstraints - 1, : nconstraints - 1, : nconstraints - 1] = inputCB.boundaryConstraints['down-bottom-left'][-degree -
                                                                                                                loffset: -nconstraints, -degree - loffset: -nconstraints, -degree - loffset: -nconstraints]
                        localAssemblyWeights[: nconstraints -
                                            1, : nconstraints - 1, : nconstraints - 1] += 1.0
                else:
                    initSol[: nconstraints, : nconstraints, : nconstraints] = inputCB.boundaryConstraints['down-bottom-left'][-degree -
                                                                                                    loffset: -nconstraints, -degree - loffset: -nconstraints, -degree - loffset: -nconstraints]
                    localAssemblyWeights[: nconstraints,
                                        : nconstraints, : nconstraints] += 1.0
            ...
            if 'down-top-right' in inputCB.boundaryConstraints:
                if oddDegree:
                    localBCAssembly[-nconstraints, -nconstraints, nconstraints - 1] += inputCB.boundaryConstraints['down-top-right'][
                        nconstraints - 1, nconstraints - 1, -nconstraints]
                    localAssemblyWeights[-nconstraints, -
                                        nconstraints, nconstraints - 1] += 1.0

                    if nconstraints > 1:
                        initSol[-nconstraints + 1:, -nconstraints + 1:, : nconstraints - 1] = inputCB.boundaryConstraints['down-top-right'][
                            nconstraints: degree + loffset, nconstraints: degree + loffset, -degree - loffset: -nconstraints]
                        localAssemblyWeights[-nconstraints +
                                            1:, -nconstraints + 1:, : nconstraints - 1] += 1.0
                else:
                    initSol[-nconstraints:, -nconstraints:, : nconstraints] = inputCB.boundaryConstraints['down-top-right'][
                        nconstraints: degree + loffset, nconstraints: degree + loffset, -degree - loffset: -nconstraints]
                    localAssemblyWeights[-nconstraints:, -
                                        nconstraints:, : nconstraints] += 1.0
