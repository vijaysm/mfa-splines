...
class ProblemSolver2D:
    ...
    def __init__(self, icBlock, coreb, xb, xl, yl, degree, augmentSpanSpace=0, useDiagonalBlocks=True):
    ...
    def lsqFit(self):
        ...
        if use_cho:
            X = linalg.cho_solve(linalg.cho_factor(
                np.matmul(self.inputCB.decodeOpXYZ['x'].T, self.inputCB.decodeOpXYZ['x'])), self.inputCB.decodeOpXYZ['x'].T)
            Y = linalg.cho_solve(linalg.cho_factor(
                np.matmul(self.inputCB.decodeOpXYZ['y'].T, self.inputCB.decodeOpXYZ['y'])), self.inputCB.decodeOpXYZ['y'].T)
            ...
            NxTQNy = np.matmul(self.inputCB.decodeOpXYZ['x'].T, np.matmul(self.inputCB.refSolutionLocal, self.inputCB.decodeOpXYZ['y']))
                    ...
                    if dir[1] > 0:  # target block is above current subdomain
                        ...
                        if verbose:
                            print("%d sending to %d" % (cp.gid(), target.gid), ' Bottom: ',
                                  inputCB.controlPointData[:, 0:1+self.degree+self.augmentSpanSpace].shape)
                            ...
                            if verbose:
                                print(
                                    "%d sending to %d" % (cp.gid(),
                                                          target.gid),
                                    ' Diagonal = right-top: ', inputCB.controlPointData
                                    [-1: -2 - self.degree - self.augmentSpanSpace: -1, : 1 + self.degree +
                                     self.augmentSpanSpace])
                        ...
                        if dir[0] < 0 and dir[1] < 0:
                            ...
                            if verbose:
                                print(
                                    ...
                                    [: 1 + self.degree + self.augmentSpanSpace, -1 - self.degree - self.augmentSpanSpace:])
                        ...
                        if dir[0] > 0 and dir[1] < 0:
                            ...
                            if verbose:
                                print(
                                    "%d sending to %d" % (cp.gid(),
                                                          target.gid),
                                    ' Diagonal = right-bottom: ', inputCB.controlPointData
                                    [: 1 + self.degree + self.augmentSpanSpace, -1 - self.degree - self.augmentSpanSpace:])
                ...
                if dir[1] > 0:  # target block is above current subdomain
                    ...
                    if verbose:
                        print("Top: %d received from %d: from direction %s" % (cp.gid(), tgid,
                              dir), inputCB.topconstraint.shape, inputCB.topconstraintKnots.shape)
                    ...
                    if verbose:
                        print("Bottom: %d received from %d: from direction %s" % (
                            cp.gid(), tgid, dir), inputCB.bottomconstraint.shape, inputCB.bottomconstraintKnots.shape)
                    ...
                    if dir[0] > 0 and dir[1] > 0:
                        ...
                        if verbose:
                            print("Top-right: %d received from %d: from direction %s" %
                                  (cp.gid(), tgid, dir), inputCB.boundaryConstraints['top-right'].shape)
                    ...
                    if dir[0] > 0 and dir[1] < 0:
                        ...
                        if verbose:
                            print("Bottom-right: %d received from %d: from direction %s" %
                                  (cp.gid(), tgid, dir), inputCB.boundaryConstraints['bottom-right'].shape)
                    ...
                    if dir[0] < 0 and dir[1] < 0:
                        ...
                        if verbose:
                            print("Bottom-left: %d received from %d: from direction %s" %
                                  (cp.gid(), tgid, dir), inputCB.boundaryConstraints['bottom-left'].shape)
                    ...
                    if dir[0] < 0 and dir[1] > 0:
                        ...
                        if verbose:
                            print("Top-left: %d received from %d: from direction %s" %
                                  (cp.gid(), tgid, dir), inputCB.boundaryConstraints['top-left'].shape)
    ...
    def initialize_solution(self, inputCB, initSol, degree, augmentSpanSpace, fullyPinned):
        ...
        if fullyPinned:
            ...
            freeBounds[1] = len(localBCAssembly[:, 0]) if inputCB.isClamped['right'] else len(
                localBCAssembly[:, 0]) - (nconstraints - 1 if oddDegree else nconstraints)
            freeBounds[2] = 0 if inputCB.isClamped['bottom'] else(
                nconstraints - 1 if oddDegree else nconstraints)
            freeBounds[3] = len(localBCAssembly[0, :]) if inputCB.isClamped['top'] else len(
                localBCAssembly[0, :])-(nconstraints-1 if oddDegree else nconstraints)
            ...
            if 'left' in inputCB.boundaryConstraints:
                if oddDegree:
                    localBCAssembly[nconstraints-1, freeBounds[2]:freeBounds[3]
                                    ] += inputCB.boundaryConstraints['left'][-nconstraints, freeBounds[2]:freeBounds[3]]
                    localAssemblyWeights[nconstraints-1,
                                        freeBounds[2]:freeBounds[3]] += 1.0

                    if nconstraints > 1:
                        initSol[:nconstraints-1, freeBounds[2]:freeBounds[3]
                                ] = inputCB.boundaryConstraints['left'][-degree - loffset:-nconstraints, freeBounds[2]:freeBounds[3]]
                        localAssemblyWeights[:nconstraints-1,
                                            freeBounds[2]:freeBounds[3]] += 1.0
                else:
            ...
            if 'right' in inputCB.boundaryConstraints:
                if oddDegree:
                    localBCAssembly[-nconstraints, freeBounds[2]:freeBounds[3]
                                    ] += inputCB.boundaryConstraints['right'][nconstraints-1, freeBounds[2]:freeBounds[3]]
                    localAssemblyWeights[-nconstraints,
                                        freeBounds[2]:freeBounds[3]] += 1.0

                    if nconstraints > 1:
                        initSol[-nconstraints + 1:, freeBounds[2]:freeBounds[3]
                                ] = inputCB.boundaryConstraints['right'][nconstraints: degree + loffset, freeBounds[2]:freeBounds[3]]
                        localAssemblyWeights[-nconstraints + 1:,
                                            freeBounds[2]:freeBounds[3]] += 1.0
                else:
                    localAssemblyWeights[-nconstraints:,
                                        freeBounds[2]:freeBounds[3]] += 1.0
                    initSol[-nconstraints:, freeBounds[2]: freeBounds[3]] = inputCB.boundaryConstraints['right'][
                        nconstraints: degree + loffset, freeBounds[2]: freeBounds[3]]

            if 'top' in inputCB.boundaryConstraints:
                if oddDegree:
                    localBCAssembly[freeBounds[0]: freeBounds[1],
                                    -nconstraints] += inputCB.boundaryConstraints['top'][
                    ...
                    if nconstraints > 1:
                        initSol[freeBounds[0]: freeBounds[1],
                                -nconstraints + 1:] = inputCB.boundaryConstraints['top'][
                            freeBounds[0]: freeBounds[1],
                            nconstraints: loffset + degree]
                        localAssemblyWeights[freeBounds[0]
                            :freeBounds[1], -nconstraints+1:] += 1.0
                else:
                    initSol[freeBounds[0]:freeBounds[1], -nconstraints:] = inputCB.boundaryConstraints['top'][freeBounds[0]                                                                                                                   :freeBounds[1], nconstraints:loffset+degree]
                    localAssemblyWeights[freeBounds[0]
                        :freeBounds[1], -nconstraints:] += 1.0
            ...
            if 'bottom' in inputCB.boundaryConstraints:
                if oddDegree:
                    localBCAssembly[freeBounds[0]:freeBounds[1],
                                    nconstraints-1] += inputCB.boundaryConstraints['bottom'][freeBounds[0]:freeBounds[1], -nconstraints]
                    localAssemblyWeights[freeBounds[0]
                        :freeBounds[1], nconstraints-1] += 1.0

                    if nconstraints > 1:
                        initSol[freeBounds[0]: freeBounds[1],
                                : nconstraints - 1] = inputCB.boundaryConstraints['bottom'][
                            freeBounds[0]: freeBounds[1],
                    ...
                    initSol[freeBounds[0]: freeBounds[1],
                            : nconstraints] = inputCB.boundaryConstraints['bottom'][
                        freeBounds[0]: freeBounds[1],
                        -degree - loffset: -nconstraints]
                    localAssemblyWeights[freeBounds[0]
                        :freeBounds[1], : nconstraints] += 1.0
