...
#             residual = np.dot(self.Nu.T, res1).reshape(self.Nu.shape[1]*self.Nv.shape[1])
            ...
            if constraints is not None and len(constraints) > 0 and constrainInterfaces:
                if len(left) > 1:
                    if useDecodedConstraints:
                        ...
                        constrained_residual_norm += ( np.sum( ( P[-1, :] - 0.5 * (constraints[-1, :] + leftdata[:]) )**2 ) / len(leftdata) )
                ...
                if len(right) > 1:
                    if useDecodedConstraints:
                        ...
                        constrained_residual_norm += ( np.sum( ( P[0, :] - 0.5 * (constraints[0, :] + rightdata[:]) )**2 ) / len(rightdata) )
                ...
                if len(top) > 1:
                    if useDecodedConstraints:
                        ...
                        constrained_residual_norm += ( np.sum( ( P[:, -1] - 0.5 * (constraints[:, -1] + topdata[:]) )**2 ) / len(topdata) ) 
                ...
                if len(bottom) > 1:
                    if useDecodedConstraints:
                        ...
                        constrained_residual_norm += ( np.sum( ( P[:, 0] - 0.5 * (constraints[:, 0] + bottomdata[:]) )**2 ) / len(bottomdata) ) 
            ...
            if verbose:
                print('Residual = ', net_residual_norm, ' and res_dec = ', decoded_residual_norm, ' and constraint = ', np.sqrt(constrained_residual_norm))
